...
\textbf{针对问题二，}将塔址、统一镜面尺寸、安装高度以及交错布局参数作为设计变量，采用容量筛选、完整光线追踪和收益排序删镜相结合的求解方法，并在仓库已有搜索检查点基础上进行多尺度坐标改进。得到 \QtwoCount\ 面统一定日镜，尺寸为 $\QtwoWidth\,\mathrm m\times\QtwoHeight\,\mathrm m$，安装高度为 $\QtwoZ\,\mathrm m$；年均输出为 \QtwoP\,$\mathrm{MW}$，单位面积年均输出为 \QtwoJ\,$\unitp$，全部几何约束通过检查。

\textbf{针对问题三，}按径向距离和方位将镜场划分为六组，交替调整镜面宽高与安装高度，结合补镜、位置交换和逐镜微调优化面积分配。最终设计包含 \QthreeCount\ 面镜，年均输出为 \QthreeP\,$\mathrm{MW}$，单位面积年均输出为 \QthreeJ\,$\unitp$，较统一镜型提高 \GainJ\%。两类设计分别经每镜每时刻 $4096$ 条光线、$4$ 组独立随机化复核，年均功率的单侧 $95\%$ 数值下界均超过额定值。
...
\begin{figure}[htbp]
\centering\includegraphics[width=.96\linewidth]{figures/q1_analysis.pdf}
...
随后依次对塔址南北坐标、宽度、高度、安装高度、环距、旋转角及两个相位施加正负坐标试探，使用 $0.02$ 和 $0.007$ 两级无量纲步长。完整追踪初评采用 $256$ 条光线，有希望提高目标的候选再用 $512$ 条光线复算；接受时要求年均功率达到 $P_*$ 且单位面积功率增加。所有接受和拒绝记录保存在随文实验数据中。

这一计算属于在已有多布局搜索结果上的继续改进，没有把未重新运行的全部历史种群记成本次实验，也没有假定参数化布局覆盖所有可能镜位。搜索结束后，采用与搜索种子不同的四组高精度采样进行最终复核。

\Needspace{12\baselineskip}
\subsection{设计结果与解释}
\input{tables/q2_design.tex}
...
\begin{figure}[htbp]
\centering\includegraphics[width=.97\linewidth]{figures/designs.pdf}
...
本次实施 $48$ 次分组试探，按组循环执行：降低镜面高度、减小镜面宽度、调整安装高度、联合改变宽高、提高安装高度并适度减小镜高。前期尺寸步长为 $0.35\,\mathrm m$，后期为 $0.15\,\mathrm m$；所有试探使用 $512$ 条光线和完整 $60$ 个时刻。安装高度自动满足 $z_i\ge h_i/2+0.1$，随后独立检查完整几何约束。

若试探提高了单位面积效率但功率不足，先检查其他镜面是否存在扩高空间，再从未占用的候选位置中补充镜面。候选优先次序由孤立单位面积产热决定，加入后重新计算全部镜间作用。若功率富余，使用收益排序删镜压缩面积。接受条件始终为
\begin{equation}
\mean P_{\rm trial}\ge P_*,\qquad J_{\rm trial}>J_{\rm current}.
\end{equation}
因此，近似收益排序只产生搜索方向，不替代最终可行性判断。

分组搜索之后，程序尝试低密度镜面与高价值候选位置交换、塔址小范围平移，以及十二次低收益单镜镜高微调。每步都重新计算几何光学效果。本次 $48$ 次分组试探中有 $5$ 次被接受，随后 $12$ 次逐镜微调均提高了目标；几何不可行的位置交换与未改善目标的塔址试探不予接受。该策略能够在维持容量的同时进一步减少低效面积，但并未穷举全部逐镜自由度。

\Needspace{12\baselineskip}
\subsection{异构设计结果与两问比较}
\input{tables/q3_design.tex}
\input{tables/q3_monthly.tex}

\begin{figure}[htbp]
\centering\includegraphics[width=.96\linewidth]{figures/heterogeneity.pdf}
\caption{异构设计的镜面面积与安装高度分布}
\label{fig:hetero}
\end{figure}

异构方案的参数范围见表\ref{tab:q3design}，空间分布见图\ref{fig:hetero}。最终形成 \QthreeTypes\ 种镜面宽高组合和 \QthreeZTypes\ 档安装高度；被接受的尺寸变化主要体现在镜面高度。其年均热功率为 \QthreeP\,$\mathrm{MW}$，单位面积年均输出为 \QthreeJ\,$\unitp$。相比问题二，单位面积目标提高 \GainJ\%，总镜面面积减少 \AreaSaving\%。这种收益来自尺寸、安装高度和镜面保留位置的共同变化，不能把所有提升归因于单一变量。
...
\begin{figure}[htbp]
\centering\includegraphics[width=.96\linewidth]{figures/monthly_power.pdf}
...
本次运行 $16$ 项自动检验，全部通过。检验涵盖太阳东西方向与时刻对称性、镜心反射定律、镜边水平、有限矩形边界、圆柱首次命中、逆向阴影查询、面积加权恒等式、镜序置换不变性，以及删去遮挡物后剩余镜功率不减。
...
\begin{figure}[H]
\centering\includegraphics[width=.93\linewidth]{figures/convergence.pdf}
...
第一，太阳盘、塔身半径和有效受光面属于显式物理假设，尚未通过实际电站数据校准。第二，优化只搜索三类参数化布局及其局部改动，且本次数值实验从已有检查点继续搜索；不能据此宣称对所有塔址、镜位和逐镜尺寸得到全局最优。第三，四组随机化提供的是有限样本的数值误差估计，对更小的优化收益应增加独立组数。
...
\section{实验数据与复现说明}
\subsection{文件与精度约定}
论文数据由当前仓库的 \texttt{heliostat} 模块直接计算，未修改核心物理公式和光线追踪程序。\texttt{final/reproduce.py} 固定本次恢复参数、搜索步长、容量裕量和独立复核种子，并将输出写入 \texttt{final/data/}。文中的表格由 \texttt{build\_assets.py} 从该目录读取，图形亦直接使用逐镜或逐时刻数组生成。
...
\texttt{q2\_search\_history.json} 等 & 本次实际试探、接受与拒绝记录\\\bottomrule
\end{tabular}
\end{table}
...
\end{verbatim}
复现脚本在已有最终镜场数组时直接重做独立验证；若要重新执行搜索，应先将已有的 \texttt{final/data/} 备份并使用空数据目录。搜索过程使用种子 $202309$，最终复核种子为 $915721$、$916718$、$917715$、$918712$。所有输入文件的 SHA-256 在运行前后比较，确保题目附件未被改写。
...
依次对统一镜型的塔址、尺寸、高度与布局参数施加正负多尺度坐标试探。\\
对每个试探生成镜场，检查约束；完整追踪后按面积收益排序删镜并重算。\\
仅接受功率达到 $P_*$ 且单位面积目标增加的方案，得到问题二设计。\\
以问题二为初始解，按径向三层和方位两类构造六组。\\
循环调整组内宽高与安装高度；容量不足时补镜，容量富余时删镜。\\
尝试位置交换、塔址小范围平移和逐镜尺寸微调，保持可行优先接受规则。\\
